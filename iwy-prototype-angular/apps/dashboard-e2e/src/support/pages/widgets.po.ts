...
export const completeNewWidgetForm = (widget) => {
  cy.get(`[data-cy=widget-form-title]`).type(widget.artistName, { delay: 20});
...
export const completeUpdateWidgetForm = (widget) => {
  cy.get(`[data-cy=widget-form-title]`).clear().type(`${widget.artistName}!!`, { delay: 20});
